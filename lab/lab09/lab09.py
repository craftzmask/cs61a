...
def is_bst(t):
    ...
    False
    """
    def bst_helper(t, func):
        if t.is_leaf():
            return t.label
        
        result = t.label
        for b in t.branches:
            result = func(result, bst_helper(b, func))
        return result
    ...
    if len(t.branches) == 2:
        max_left = bst_helper(t.branches[0], max)
        min_right = bst_helper(t.branches[1], min)
        return max_left <= t.label and min_right > t.label
