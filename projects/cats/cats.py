...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    picked = [p for p in paragraphs if select(p)]
    if k >= len(picked):
        return ''
    else:
        return picked[k]
...
def accuracy(typed, source):
    ...
    if not typed_words and not source_words:
        ...
        for t, s in zip(typed_words, source_words):
            if t == s:
                corrects += 1
        return corrects / len(typed_words) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    result = min(word_list, key=lambda w: diff_function(typed_word, w, limit))
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            if time(match, player_index, word_index) < fastest_time:
                ...
                fastest_index = player_index
        result[fastest_index].append(get_word(match, word_index))
    return result
    # END PROBLEM 10
